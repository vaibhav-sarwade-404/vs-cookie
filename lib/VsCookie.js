...
var isValidCookieValue = function (cookieValue, separator) {
    if (cookieValue.includes(separator)) {
        throw new TypeError("cookie value cannot conatain separator(".concat(separator, ")."));
    }
    var signatureSeparatorRegex = new RegExp(signatureSeparator, "g");
    var matchResult = cookieValue.match(signatureSeparatorRegex);
    if (Array.isArray(matchResult) && matchResult.length) {
        throw new TypeError("cookie value cannot conatain signature separator(".concat(signatureSeparator, ")."));
    }
    return true;
};
var VsCookie = /** @class */ (function () {
    ...
    VsCookie.getCookie = function (cookies, cookieName, options) {
        ...
        var cookie = Array.isArray(cookieNameValuePair)
            ...
            : "";
        var _separator = (options === null || options === void 0 ? void 0 : options.separator) || separator;
        if (secret) {
     ...
     *
     */
    VsCookie.sign = function (cookie, secret, options) {
        ...
        var _seperate = options.separator || separator;
        var encode = options.encode || encodeURIComponent;
        isValidCookieValue(cookie, _seperate);
        return encode("".concat(cookie).concat(_seperate).concat(VsCookie.hash("".concat(cookie).concat(signatureSeparator).concat(secret), secret)));
    };
    /**
     *
     ...
     * @throws {TypeError | Error}
     */
    VsCookie.verify = function (cookie, secret, options) {
        if (options === void 0) { options = {
        ...
        var _a = __read(cookie.split(_seperate), 2), originalCookieVal = _a[0], originalCookieHash = _a[1];
        if (!originalCookieVal || !originalCookieHash) {
            return false;
        }
        var cookieHash = VsCookie.hash("".concat(originalCookieVal).concat(signatureSeparator).concat(secret), secret);
